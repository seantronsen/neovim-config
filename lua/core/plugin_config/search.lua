...
-- new bindings to search relative to the current buffer position.
-- created because I got tired of having to exit to have an easier time searching plugin package directories.
...
vim.keymap.set(
	"n",
	"<leader>fpf",
	path_picker_builder(builtin.find_files),
	{ desc = "[f]ind [f]iles (relative to [p]ath)" }
)

vim.keymap.set(
	"n",
	"<leader>fpg",
	path_picker_builder(builtin.live_grep),
	{ desc = "[f]ind with live [g]rep (relative to [p]ath)" }
)
